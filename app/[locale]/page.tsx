...
const { data: { session: rootSession } } = await supabase.auth.getSession();
  
  if (rootSession?.user) {
    // ✅ Changed data.user.id to rootSession.user.id
    const { data: profile } = await supabase.from('profiles').select('role').eq('id', rootSession.user.id).single();
